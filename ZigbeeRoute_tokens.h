...
#include "ConfigDevice.h"
	...
	// Creator for attribute: on/off, endpoint: 7
	#define CREATOR_ON_OFF_7 0xB007
	// Creator for attribute: on/off, endpoint: 15
	#define CREATOR_ON_OFF_15 0xB008
	...
	DEFINE_BASIC_TOKEN(ON_OFF_7, tokType_on_off, 0x00)
	DEFINE_BASIC_TOKEN(ON_OFF_15, tokType_on_off, 0x00)
	#endif // DEFINETOKENS
	...
	#define GENERATED_TOKEN_LOADER(endpoint) do {\
	  ...
	  if((endpoint) == 7 || ((endpoint) == EMBER_BROADCAST_ENDPOINT && epNetwork == curNetwork)) { \
	    halCommonGetToken((tokType_on_off *)ptr, TOKEN_ON_OFF_7); \
	    emberAfWriteServerAttribute(7, ZCL_ON_OFF_CLUSTER_ID, ZCL_ON_OFF_ATTRIBUTE_ID, (int8u*)ptr, ZCL_BOOLEAN_ATTRIBUTE_TYPE); \
	  } \
	  ...
	  if((endpoint) == 15 || ((endpoint) == EMBER_BROADCAST_ENDPOINT && epNetwork == curNetwork)) { \
	    ...
	    emberAfWriteServerAttribute(15, ZCL_ON_OFF_CLUSTER_ID, ZCL_ON_OFF_ATTRIBUTE_ID, (int8u*)ptr, ZCL_BOOLEAN_ATTRIBUTE_TYPE); \
	  } \
	} while(FALSE)
	...
	#define GENERATED_TOKEN_SAVER do {\
	  ...
	  if ( endpoint == 1 ) { \
	    ...
	    if ( clusterId == 0x06 ) { \
	      if ( metadata->attributeId == 0x0000 && !emberAfAttributeIsClient(metadata) ) \
	        halCommonSetToken(TOKEN_ON_OFF_7, data); \
	    } \
	  } else if ( endpoint == 15) { \
	    if ( clusterId == 0x06 ) { \
	      if ( metadata->attributeId == 0x0000 && !emberAfAttributeIsClient(metadata) ) \
	...
	#define CREATOR_ON_OFF_1 0xB004
	// Creator for attribute: on/off, endpoint: 15
	#define CREATOR_ON_OFF_15 0xB005
	...
	#define CREATOR_ON_OFF_3 0xB005
	// Creator for attribute: on/off, endpoint: 15
	#define CREATOR_ON_OFF_15 0xB006
